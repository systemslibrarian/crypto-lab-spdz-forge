 ...
 *
 * WHAT THIS FINDS ON SPDZ FORGE, and what it deliberately does not.
 *
 ...
 *
 * One 1.4.11 case on this page is out of reach of BOTH lists here and is
 * measured separately by `auditMeter` in `gate.ts`: `div.meter`, the triple-bank
 * gauge. `role="meter"` is not an interactive role, so it is not in `CONTROL`
 * below, and its edges are not text, so `contrast.ts` cannot see them either —
 * yet the fill says how many triples are left and the track boundary says out of
 * how many, which is exactly the "part of a graphic required to understand the
 * content" the criterion names.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks five panels of full-precision field elements,
     * each in its own `<code class="fe">`, so a single state is hundreds of
     * sibling elements all re-walking the same ancestors up to `<body>`.
