 ...
 *
 *  - TEXT OVER A SURFACE AXE DECLINES TO RESOLVE. This page paints three of its
 *    most load-bearing surfaces with `color-mix()`, and Chromium hands that back
 *    to `getComputedStyle` unresolved, so axe files every one of them under
 *    `incomplete`. They are: `tr.you-row`
 *    (`color-mix(in oklab, var(--accent) 14%, transparent)`), which is the
 *    tampered party's row — the single row the entire MAC exhibit is about;
 *    every `.chip` (`color-mix(in oklab, currentColor 10%, transparent)`), which
 *    is how every verdict on this page is rendered, ok/alarm/warn/neutral alike;
 *    and `.cl-hero-why` (`color-mix(in oklab, var(--accent) 6%, transparent)`).
 *    A violations-only gate therefore measured the contrast of almost nothing
 *    this lab concludes.
 ...
 *    at `.85` and `#app button:disabled` at `.45`. The second is exempt (WCAG
 *    excuses inactive components, and the walk below honours that), but the
 *    first is the page's subtitle and is measured for real: `--text` at 85% over
 *    `--bg` is rgb(198, 202, 204) in dark and rgb(66, 64, 61) in light.
 *
 ...
 *    boxes are the three `.scroll-x` wrappers (`overflow-x: auto`), which is how
 *    every share table on this page is presented and which genuinely overflows
 *    at 380px — four columns of full-precision 19-digit field elements will not
 *    fit in a phone — and `.meter` (`overflow: hidden`). Content scrolled past
 ...
 *    draws no figures of its own — the party cards, the public-wire box and the
 *    two-protocol comparison are all HTML and CSS grid — so the sibling walk
 *    finds nothing to do here. It is kept because the alternative is a helper
 ...
 * In particular the `<details>` disclosures here keep the user agent's own
 * marker — the stylesheet sets `cursor` and `font-weight` on `summary` and
 * nothing else — so unlike most labs in this fleet there is no author-painted
 * `?` or `▸` glyph carrying the affordance.
 *
 * SECOND, `filter`. `#app button:hover` used to declare
 * `filter: brightness(1.12)`, and a CSS filter is applied by the compositor
 * AFTER this walk has read `color` and `background-color` — so the walk, and
 * axe, both measured the resting button and neither could see the hovered one.
 * That gap was real: brightening `--accent` #b45309 by 1.12 in sRGB gives
 * rgb(202, 93, 10), against which the button's own `#ffffff` ink measures
 * 4.14:1, under the 4.5:1 it cleared at rest — in BOTH themes, since `--accent`
 * is the same value in both. It was fixed at the source rather than modelled
 * here (see `--accent-hover` in `style.css`), because a filter can move any
 * property this walk reads and modelling one instance would not make the class
 * measurable.
 */
 ...
 * uses. The one genuinely short-lived styling here is `.fresh`, which the Beaver
 * stepper puts on the rows a step just produced and which runs
 * `fresh-flash 1.4s` from an accent tint to `transparent` — and under the
 * reduced motion this gate asserts, `#app *` cancels it outright
 * (`animation: none !important`), so the class is present on six consecutive
 * driven states while painting nothing transient, and a scan cannot catch a
 * half-faded row.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives 1.4s is shorter than a full axe
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks five panels, and the expensive part is that every
     * value this lab shows is a full-precision field element in its own
     * `<code class="fe">` — three parties x up to six rows of them in the Beaver
     * stepper, plus three- and four-column share tables in two other panels — so
     * a single state is hundreds of sibling elements all re-walking the same
     ...
     * that is the three `.scroll-x` share-table wrappers (`overflow-x: auto`),
     * which really do overflow at 380px because a share table is four columns of
     * 19-digit field elements, and `.meter` (`overflow: hidden`), which clips
     * its own fill bar to the rounded track.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. Here that is all five
      // disclosures that ship shut (the jargon primer, three in the expert
      // section, and the MAC panel's advanced ordering exhibit) plus the two
      // that only exist once a panel has produced output. The gate opens each by
      // clicking its <summary>, which is the route a reader has, rather than
      // setting `.open` from script — which is exactly what the spec this
      // replaces did to every one of them at once, before its single scan.
      ...
      //
      // THIS LAB DEFINES NO `.sr-only`, and the guard is kept anyway. The one
      // piece of visually-hidden text on the page — the shared header's
      // `.cl-skip-link` — uses the OTHER idiom, parking at `top: -3rem`, which
      // the document-space check just above already handles. The clip idiom is
      // what a contributor reaches for the moment they want a screen-reader-only
      // label, and it fails in the direction that FABRICATES failures: the box
      // has a rect and passes `checkVisibility()` while the compositor draws
      // exactly zero pixels of it, so a walk without this guard reports a ratio
      // for ink that was never laid down. It is deliberately narrow: only a
     ...
     *
     * What this lab hides is a closed set with exactly two members, and both are
     * decorative. `dom.ts`'s `chip()` wraps its leading glyph — ✓, ✗, ⚠ or ▸ —
     * in `<span aria-hidden="true">`, which is right: the glyph is the
     * non-colour cue and the words immediately after it say the same thing, in
     * the same ink on the same surface, so measuring the chip measures the
     * glyph. The other is the shared header's two SVG marks. Nothing else on the
     * page carries the attribute.
     *
     * Nothing here hides a VALUE. No share, no MAC, no opened field element, no
     * verdict and no Σσᵢ is inside an `aria-hidden` subtree — which was checked
     ...
     *
     * This lab has no SVG of its own — the party cards, the public-wire box and
     * the two-protocol comparison are all HTML and CSS grid, and its only SVG is
     * the shared header's two `aria-hidden` marks. The guard stays because the
     * usual way the mistake arrives is exactly how this lab builds its DOM:
     * `dom.ts`'s `h()` spreads a `children` array straight into `append()`, and
     * a `null` or a stray array joined to a string lands as a bare text node in
     * whatever element wraps it — inside a `<g>` that renders nothing, while a
     * walk that measured it would report a 1:1 failure for ink never laid down.
     */
     ...
     * this page runs seven sections deep and the day a `height: 100%` appears on
     * either box — or the day `html`'s background is dropped as redundant —
     * everything below the first screen would report against a white page that
     * does not exist, in a lab whose dark palette is `#101214`.
     */
